        ...
        def GetOpenGLCaption(self, GameData):
            try:
                if self.Devmode >= 5 and self.Devmode <= 9:
                    ...
                    GameData.wnd.title = f"Pycraft: v{self.version}: Playing | Developer Mode | Pos: {round(self.X, 2)}, {round(self.Y, 2)}, {round(self.Z, 2)} | V: {self.Total_move_x}, {self.Total_move_y}, {self.Total_move_z} | FPS: {self.FPS} eFPS: {int(self.eFPS)} aFPS: {int(self.aFPS/self.Iteration)} Iteration: {self.Iteration} | MemUsE: {self.mod_Psutil__.virtual_memory().percent}% | CPUUsE: {self.mod_Psutil__.cpu_percent()}% | Theme: {self.theme} | Thread Count: {self.mod_Threading__.active_count()}"
                else:
